...
def build_base_image(framework_name, framework_version, processor, base_image_tag, cwd="."):
    ...
    subprocess.check_call(
        ["docker", "build", "-t", base_image_uri, "-f", dockerfile_location, build_directory], cwd=cwd
    )
...
def build_image(py_version, framework_name, framework_version, processor, tag, cwd="."):
    ...
    dockerfile_location = os.path.join(
        "docker", framework_version, "final", py_version, "Dockerfile.{}".format(processor)
    )
...
def create_docker_command(tmpdir, use_gpu=False, detached=False):
    ...
    command = ["docker-compose", "-f", os.path.join(tmpdir, DOCKER_COMPOSE_FILENAME), "up", "--build"]
...
def create_training(
    ...
    print("training dir: \n{}".format(str(subprocess.check_output(["ls", "-l", tmpdir]).decode("utf-8"))))
...
def create_input_data_config(data_path, customer_script):
    ...
    if customer_script:
        ...
        config = {
            c: {"ContentType": "libsvm", "S3DistributionType": "FullyReplicated", "TrainingInputMode": "File"}
            for c in channels
        }
...
def create_docker_services(
    ...
    return {
        h: create_docker_host(
            tmpdir, h, image, environment, optml_dirs, command, additional_volumes, entrypoint, use_gpu
        )
        for h in hosts
    }
...
def create_docker_host(
    tmpdir, host, image, environment, optml_subdirs, command, volumes, entrypoint=None, use_gpu=False
):
...
def credentials_to_env(session):
    try:
        ...
        credentials_list = ["AWS_ACCESS_KEY_ID=%s" % (str(access_key)), "AWS_SECRET_ACCESS_KEY=%s" % (str(secret_key))]
        if session_token:
...
def create_docker_compose(
    ...
    services = create_docker_services(
        command,
        tmpdir,
        hosts,
        image,
