...
def dimension_reduction_target_validator(value, dependencies):
    ...
    if dimension_reduction_type == "none" and value is not None:
        raise ValueError(
            "Hyperparameter 'dimension_reduction_target' should only be set when " "'dimension_reduction_type' is set."
        )
...
def initialize():
    return hpv.Hyperparameters(
        ...
        hpv.IntegerHyperparameter(
            name="k", range=hpv.Interval(min_open=0, max_closed=1024), required=True, tunable=True
        ),
        ...
        hpv.IntegerHyperparameter(
            name="dimension_reduction_target",
        ...
        hpv.CategoricalHyperparameter(
            name="index_type", range=["faiss.Flat", "faiss.IVFFlat", "faiss.IVFPQ"], default="faiss.Flat"
        ),
        hpv.CategoricalHyperparameter(name="index_metric", range=["L2", "INNER_PRODUCT", "COSINE"], default="L2"),
        hpv.IntegerHyperparameter(
            name="faiss_index_pq_m",
...
class TestHyperparameters(unittest.TestCase):
    def test_simple_dependency(self):
        ...
        hps = hpv.Hyperparameters(
            hpv.Hyperparameter(name="a", dependencies=validate, required=True),
    ...
    def test_simple_integral(self):
        hps = hpv.Hyperparameters(
            hpv.IntegerHyperparameter(name="a", range=hpv.Interval(min_open=0, max_closed=1), required=False)
        )
    ...
    def test_simple_categorical(self):
        hps = hpv.Hyperparameters(
            hpv.CategoricalHyperparameter(name="a", range=["classifier", "regressor"], required=True)
        )
        with self.assertRaises(exc.UserError):
            hps.validate({})
    ...
    def test_simple_continuous(self):
        hps = hpv.Hyperparameters(
            hpv.ContinuousHyperparameter(name="gamma", range=hpv.Interval(min_closed=0, max_closed=1), required=True)
        )
        with self.assertRaises(exc.UserError):
            hps.validate({})
    ...
    def test_complex_knn(self):
        hps = initialize()
        result = hps.validate({"feature_dim": "4", "k": "100", "predictor_type": "regressor", "sample_size": "256"})
    ...
    def test_integer_tunable_range(self):
        hyperparameter = hpv.IntegerHyperparameter(
            ...
            range=hpv.Interval(min_open=0),
            tunable=True,
            tunable_recommended_range=hpv.Interval(min_open=0, max_open=100, scale=hpv.Interval.LINEAR_SCALE),
            required=True,
        )
        self.assertEqual(
            hyperparameter.format_tunable_range(),
            {"IntegerParameterRanges": [{"MaxValue": "100", "MinValue": "0", "Name": "x", "ScalingType": "Linear"}]},
        )

    def test_categorical_tunable_range(self):
        hyperparameter = hpv.CategoricalHyperparameter(
            name="x", range=["a", "b", "c"], tunable=True, tunable_recommended_range=["a", "b"], required=True
        )
        self.assertEqual(
            hyperparameter.format_tunable_range(), {"CategoricalParameterRanges": [{"Name": "x", "Values": ["a", "b"]}]}
        )
    ...
    def test_continuous_tunable_range(self):
        ...
        self.assertEqual(
            hyperparameter.format_tunable_range(),
            {"ContinuousParameterRanges": [{"MaxValue": "100", "MinValue": "0", "Name": "x", "ScalingType": "Linear"}]},
        )

    def test_simple_nested_list(self):
        hps = hpv.Hyperparameters(
            hpv.NestedListHyperparameter(name="list_", range=hpv.Interval(min_closed=1, max_closed=10), required=False)
        )
    ...
    def test_multiple_alias(self):
        hps = hpv.Hyperparameters(
            hpv.ContinuousHyperparameter(
                name="a", default=0.5, range=hpv.Interval(min_closed=0, max_closed=1), required=True
            )
        )
    ...
    def test_empty_alias(self):
        hps = hpv.Hyperparameters(
            hpv.ContinuousHyperparameter(name="a", range=hpv.Interval(min_closed=0, max_closed=1), required=False)
        )
