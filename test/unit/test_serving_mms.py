...
from sagemaker_xgboost_container import serving, serving_mms
...
@patch.dict(os.environ, {"SAGEMAKER_MULTI_MODEL": "True", "XGBOOST_MMS_CONFIG": TEST_CONFIG_FILE})
@patch("sagemaker_xgboost_container.serving_mms.model_server.start_model_server")
...
def test_multi_model_user_mode_hosting_error(
    import_module, user_module_name, module_dir, start_model_server, mock_set_mms_config_file, mock_set_multi_model_env
):
    serving.serving_entrypoint()
    start_model_server.assert_called_with(
        is_multi_model=True, handler_service="sagemaker_xgboost_container.handler_service", config_file=TEST_CONFIG_FILE
    )
