...
ALGORITHM_HOME_DIR = os.path.dirname(path)
...
def submit_yarn_job(train_cfg, host_ip, num_hosts):
    ...
    logging.info("Running command: '{}'".format(cmd_submit))

    p = subprocess.Popen(cmd_submit, shell=True, stdout=FNULL)
    ...
    if p.returncode != 0:
        raise AlgorithmError("Yarn job failed!")
