...
def get_gunicorn_args(env, gunicorn_bind_address, pythonpath, module_app) -> list:
    subprocess_args = ["gunicorn"]
    ...
    subprocess_args.extend(["--log-level", os.getenv(sm_env_constants.SAGEMAKER_GUNICORN_LOG_LEVEL, "info")])
    subprocess_args.extend(["--keep-alive", os.getenv(sm_env_constants.SAGEMAKER_GUNICORN_KEEP_ALIVE, "2")])
    subprocess_args.append(module_app)
...
def start(module_app):
    ...
    gunicorn_args = get_gunicorn_args(env, gunicorn_bind_address, pythonpath, module_app)
    gunicorn = subprocess.Popen(gunicorn_args)
