...
def predict(model, model_format, dtest, input_content_type, objective=None):
    ...
    if bst_format == PKL_FORMAT:
        x = len(bst.feature_names) if bst.feature_names is not None else bst.num_features()
        y = len(dtest.feature_names) if dtest.feature_names is not None else dtest.num_features()
